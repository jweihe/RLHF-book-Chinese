...
“拒绝采样”一词源自计算统计学 [@gilks1992adaptive]，原意是：当目标分布复杂且无法直接采样时，先从易采样的分布中采样，再根据目标分布与提议分布的密度比及包络常数确定接受概率。本章常说的奖励筛选或 Best-of-N 微调借用了这一名称，并不自动具备统计学拒绝采样的精确分布保证。
...
也可从所有 prompt-补全对中选出得分最高的 K 个，但需谨慎：成对偏好训练只约束同一 prompt 内的分数差，不保证不同 prompt 的绝对分数可比较。全局筛选可能改变 prompt 分布。
先将$R$展平成一维向量：
...
$$S(R) = [\arg\max_{j} r_{i,j} \text{ for } i \in \{1,\ldots,5\}]$$
