...
二元奖励每次只提供有限的反馈信息，但这不意味着 RL 只需少量训练样本；训练可能需要大量回答采样与奖励评估。
...
OpenAI o1 的公开介绍强调了强化学习对推理的作用 [@openai2024o1]；DeepSeek-R1 技术报告则明确介绍了基于规则的奖励设计 [@guo2025deepseek]。二者公开的训练细节不同，不能据此假定它们使用完全相同的 RLVR 流程。
...
更大的上下文与生成预算可容纳更长的推理过程，但长输出本身不保证正确性，也不是所有推理任务的必要条件。
...
一些强化微调流程会反复使用同一组问题，并不断重新采样回答。更新步数、采样轮次与遍历固定数据的 epoch 不是同一概念；训练预算应由验证集表现、泛化与过拟合风险决定，而不是套用固定的数百或数千轮。
